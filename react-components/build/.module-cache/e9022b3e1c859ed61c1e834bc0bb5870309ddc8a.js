...
var Parameter = React.createClass({displayName: 'Parameter',
  ...
  render: function() {
    return (React.DOM.span({className: "RegisterMachine-Parameter", tabindex: "0"}, 
      ...
      React.DOM.span({className: "RegisterMachine-Parameter-Value", onClick: edit }, 
...
var Instruction = React.createClass({displayName: 'Instruction',
  render: function() {
    ...
    if( type === RegisterMachine.DEB || type === RegisterMachine.INC ) {
      contents.push(Parameter({name: "which", initialValue:  instruction.which}));
      contents.push(Parameter({name: "next", initialValue:  instruction.next}));
    }
    
    // only DEB has 'branch' parameter
    if( type === RegisterMachine.DEB ) {
      contents.push(Parameter({name: "branch", initialValue:  instruction.branch}));
    }
